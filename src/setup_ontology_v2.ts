...
export async function read_in_table({
  pgClient,
  offset,
  limit,
  entity_names,
  ...
  // Get ids from names... if podcast names are provided, get the required names...
  // Then pull the required entities
  // Then then recursively call function to pull the other types reuquired...
  // Just need to store what has been pulled so far so that we dont repull the same table multiple times.

  const non_rel_properties_select = "";
  for (const value_field in entity_breakdown.value_fields) {

  }

  // For person -> COALESCE(p.editor_description, p.description) AS description
  // For platforms -> normalized_name as name
  // For topics -> t.logo as cover
  ...
  // Add value fields
  for (const field of entity_breakdown.value_fields) {
    selectFields.push(`${tableAlias}.${field}`);
  }

  // Add image-type relations (like avatar)
  for (const rel of entity_breakdown.relations) {
    if (rel.image) {
      // assumes DB column is same name as relation type (e.g. avatar → logo or avatar column)
      // If you need custom mapping, just adjust this line
      selectFields.push(`${tableAlias}.${rel.type} as ${rel.type}`);
    }
  }
  ...
  const entities = await pgClient.query(`
      SELECT 
          p.id, p.name, p.description, p.logo as avatar, p.is_explicit, p.rss_feed_url,
          COALESCE(
            json_agg(
