...
const plan = await findDuplicateEpisodes({ sinceIso: new Date(since).toISOString() });
console.log(`scanned=${plan.scanned} groups=${plan.groups.length} surplus=${plan.surplus} review=${plan.review.length}`);
for (const g of plan.groups.slice(0, 15)) console.log(`  PRUNE "${g.name.slice(0, 55)}" keep=${g.keep.slice(0, 8)} prune=[${g.prune.map(p => p.slice(0, 8)).join(',')}]`);
