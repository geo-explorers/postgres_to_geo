...
async function read_in_tables_working({
  client,
  offset,
  limit,
}: {
...
async function read_in_tables({
  ...
  tables: {
    projects,
    investment_rounds: groupInvestmentRoundsBySource(investment_rounds),
    tags,
    types,
    assets,
    platforms,
  ...
  newOffset: investment_rounds.length, // increment by count of original chunk
};
}
...
const main = async () => {
    ...
    try {
        while (true) {
          ...
          const result = await read_in_tables({
            client: pgClient,
            offset,
            limit,
          });

          tables = result.tables;         // mutable assignment
          newOffset = result.newOffset;   // capture how much to increment
